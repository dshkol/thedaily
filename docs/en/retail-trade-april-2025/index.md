...
current <- total_retail %>% filter(REF_DATE == "2025-04") %>% pull(VALUE)
previous <- total_retail %>% filter(REF_DATE == "2025-03") %>% pull(VALUE)
mom_change <- (current - previous) / previous * 100

# Year-over-year change
year_ago <- total_retail %>% filter(REF_DATE == "2024-04") %>% pull(VALUE)
