...
current <- total_retail %>% filter(REF_DATE == "2022-07") %>% pull(VALUE)
previous <- total_retail %>% filter(REF_DATE == "2022-06") %>% pull(VALUE)
mom_change <- (current - previous) / previous * 100

# Year-over-year change
year_ago <- total_retail %>% filter(REF_DATE == "2021-07") %>% pull(VALUE)
...
provincial <- retail %>%
  filter(`North American Industry Classification System (NAICS)` == "Retail trade [44-45]",
         REF_DATE %in% c("2022-07", "2021-07"),
         `Adjustments` == "Seasonally adjusted",
         GEO != "Canada") %>%
  select(GEO, REF_DATE, VALUE) %>%
  pivot_wider(names_from = REF_DATE, values_from = VALUE) %>%
  mutate(yoy_change = (`2022-07` - `2021-07`) / `2021-07` * 100)
```
