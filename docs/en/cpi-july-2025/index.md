...
The Consumer Price Index (CPI) rose 1.7% in July 2025 compared with the same month a year earlier.
...
- Inflation moderated from 2.6% in February 2025 to 1.7% in July
...
current <- national_cpi %>% filter(REF_DATE == "2025-07") %>% pull(VALUE)
previous <- national_cpi %>% filter(REF_DATE == "2024-07") %>% pull(VALUE)
...
provincial <- cpi %>%
  filter(`Products and product groups` == "All-items",
         REF_DATE %in% c("2025-07", "2024-07"),
         GEO != "Canada") %>%
  select(GEO, REF_DATE, VALUE) %>%
  pivot_wider(names_from = REF_DATE, values_from = VALUE) %>%
  mutate(yoy_change = (`2025-07` - `2024-07`) / `2024-07` * 100)
```
...
display(createSidebar(articles, "cpi-july-2025", "en"));
```
