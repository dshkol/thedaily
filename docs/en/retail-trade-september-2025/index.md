---
title: Retail sales down 0.9% in September 2025
verification_json: output/data_20_10_0056_enhanced.json
toc: false
---
# Retail sales down 0.9% in September 2025

<p class="release-date">Released: November 22, 2025</p>

<div class="highlights">

- Retail sales decreased 0.9% to $69.6 billion in September 2025
- This followed a 0.6% decline in August
- Year over year, retail sales were up 3.1%

</div>

Retail sales fell 0.9% to $69.6 billion in September 2025, following a 0.6% decline in August. On a year-over-year basis, retail trade increased 3.1% compared with September 2024, when sales totalled $67.5 billion.

The September decline marked the second consecutive monthly decrease, though retail sales remained well above year-ago levels.

## Sales trend

Retail sales have fluctuated through 2025, reaching a peak of $70.2 billion in August before declining in subsequent months.
...
Retail sales have been volatile in 2025, with the September decline following a period of mixed results.
...
|-----------|---------------:|-------------------:|---------------------------:|
| Retail sales ($ billions) | 69.6 | -0.9% | +3.1% |
...
current <- total_retail %>% filter(REF_DATE == "2025-09") %>% pull(VALUE)
previous <- total_retail %>% filter(REF_DATE == "2025-08") %>% pull(VALUE)
mom_change <- (current - previous) / previous * 100

# Year-over-year change
year_ago <- total_retail %>% filter(REF_DATE == "2024-09") %>% pull(VALUE)
...
provincial <- retail %>%
  filter(`North American Industry Classification System (NAICS)` == "Retail trade [44-45]",
         REF_DATE %in% c("2025-09", "2024-09"),
         `Adjustments` == "Seasonally adjusted",
         GEO != "Canada") %>%
  select(GEO, REF_DATE, VALUE) %>%
  pivot_wider(names_from = REF_DATE, values_from = VALUE) %>%
  mutate(yoy_change = (`2025-09` - `2024-09`) / `2024-09` * 100)
```
...
display(createSidebar(articles, "retail-trade-september-2025", "en"));
```
